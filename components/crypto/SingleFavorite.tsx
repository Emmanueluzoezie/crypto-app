...
const SingleFavorite = ({crypto}: any) => {
    const [favoriteName, setFavoriteName] = useState<string[]>([])
    const { currencySymbol, currency } = useContextState()

    const { name, id, rank, allTimeHigh, total_market, image, price, symbol, market_cap, percent } = crypto
    ...
    const { data: favoriteCrypto, loading, error } = useQuery(GET_ALL_FAVORITE_CRYPTO, {
        variables: {
    ...
    const RomoveFavorite = async () => {
        ...
        try {
                ...
                toast.success("Successfully removed...", { id: notification })
            }
        catch (error) {
    ...
    return (
        <div className='w-screen'>
            <div className='flex border-t-2 items-center hover:bg-gray-100 hover:shadow-xl p-3 w-inherit cursor-pointer'>
                {/* <Link href={`/crypto/${id}`}> */}
                <div className=' w-[15%] md:w-[12%] flex justify-center md:justify-between md:px-4 md:pr-10 items-center' onClick={() => router.push(`/crypto/${nameLowerCase}`)}>
                ...
                {/* </Link> */}
                {/* <Link href={`/crypto/${id}`}> */}
                <div className='md:flex items-center md:space-x-3 px-2 font-mono w-[35%] md:w-[32%]  md:text-xl truncate' onClick={() => router.push(`/crypto/${nameLowerCase}`)}>
                ...
                {/* </Link> */}
                <div className=' pl-2 font-mono w-[65%] md:w-[54%] flex truncate' onClick={() => router.push(`/crypto/${nameLowerCase}`)}>
                    <div className='flex items-center md:space-x-3 truncate w-full'>
                        <div className=' w-[25%] sm:pl-8 md:pl-0 md:w-[12%] truncate border-black'>
                ...
                <div className='w-[13%] md:w-[10%] truncate border-black flex justify-center'>
                    <h1 className='font-semibold text-[25px] hover:text-red-600 m-0 truncate' onClick={() => RomoveFavorite()}><MdDelete /></h1>
                </div>
            </div>
        </div>
    )
